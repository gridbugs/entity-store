...
impl Component {
    fn from_input(field_name: &str, c: &input::Component) -> Result<Self> {
        let storage_type = if let Some(s) = c.storage.as_ref() {
            if let Some(s) = StorageType::from_str(s.as_str()) {
                ...
                return Err(Error::InvalidStorageType(valid_storage_type_strings()));
            }
        } else {
            None
        };

        let name = c.name_override
            .as_ref()
            .cloned()
            .unwrap_or_else(|| name_from_field_name(field_name));
    ...
    fn to_output(
        ...
        let storage = self.storage_type.as_ref().map(|s| output::StorageInfo {
        ...
        let tracked_by_spatial_hash = if let Some(spatial_hash) = spatial_hash {
            spatial_hash.tracked_components.contains(key) || spatial_hash.position_component == key
        } else {
            false
        };
        output::Component {
...
impl SpatialHashField {
    fn from_input(
        ...
        let aggregate_type = if let Some(a) = f.aggregate.as_ref() {
            if let Some(a) = AggregateType::from_str(a.as_str()) {
                ...
                return Err(Error::InvalidAggregateType(valid_aggregate_type_strings()));
            }
        } else {
            None
        };
    ...
    fn to_output(
        &self,
        key: &str,
        components: &BTreeMap<String, output::Component>,
    ) -> output::SpatialHashField {
        let aggregate = self.aggregate_type
            .map(|a| output::AggregateInfo {
                typ: a.to_str().to_string(),
                rust_type: a.to_type(self.component.typ.as_ref(), self.typ.as_ref()),
            })
            .unwrap_or_else(|| output::AggregateInfo {
                typ: "void".to_string(),
                rust_type: "()".to_string(),
            });
...
impl Spec {
    pub fn from_str(s: &str) -> Result<Self> {
        ...
        let components: Result<BTreeMap<String, Component>> = spec_in
            .components
            .iter()
            .map(|(f, c_in)| Component::from_input(f.as_str(), c_in).map(|c| (f.clone(), c)))
            .collect();
        ...
        let spatial_hash = if let Some(spatial_hash) = spec_in.spatial_hash.as_ref() {
            let spatial_hash_fields: Result<BTreeMap<String, SpatialHashField>> = spatial_hash
                .iter()
                .map(|(f, shf_in)| {
                    SpatialHashField::from_input(&shf_in, &components).map(|shf| (f.clone(), shf))
                })
                .collect();
            let spatial_hash_fields = spatial_hash_fields?;

            let tracked_components = spatial_hash_fields
                .values()
                .map(|v| v.component.key.clone())
                .collect::<BTreeSet<_>>();
        ...
        if !valid_id_widths.contains(&spec_in.id_width) {
            return Err(Error::InvalidIdWidth(
                valid_id_widths.iter().cloned().collect(),
            ));
        }
    ...
    pub fn to_output(&self) -> output::Spec {
        let components: BTreeMap<String, output::Component> = self.components
            .components
            .iter()
            .enumerate()
            .map(|(i, (k, v))| (k.clone(), v.to_output(k, i, self.spatial_hash.as_ref())))
            .collect();

        let spatial_hash = if let Some(spatial_hash) = self.spatial_hash.as_ref() {
            let fields: BTreeMap<String, output::SpatialHashField> = spatial_hash
                .fields
                .iter()
                .map(|(k, f)| (k.clone(), f.to_output(k, &components)))
                .collect();
            let position_component = components
                .get(&spatial_hash.position_component)
                .cloned()
                .unwrap();
            let mut by_component = BTreeMap::new();
            for (f, g) in izip!(fields.values(), spatial_hash.fields.values()) {
                let current = by_component
                    .entry(f.component.key.clone())
